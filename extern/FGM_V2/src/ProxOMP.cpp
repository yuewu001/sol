...
void ProxOMP::calculate_wx_gwx(double s)
{
	...
	for (i=0; i+4<prob->l; i+=4)
	{
		for (k = 0; k < num_kernel; k++)
		{
			...
			//wx_gwx[k][i+4] = wtTxt[k][i+4] - s*gw_wtTxt[k][i+4];
			//wx_gwx[k][i+5] = wtTxt[k][i+5] - s*gw_wtTxt[k][i+5];
			//wx_gwx[k][i+6] = wtTxt[k][i+6] - s*gw_wtTxt[k][i+6];
			//wx_gwx[k][i+7] = wtTxt[k][i+7] - s*gw_wtTxt[k][i+7];
		}
	}
	while(i<prob->l)
	{
...
double ProxOMP::calculate_gradient(double *gw, double *v)
{
	int m = prob->l;
	int i, k;
	double pred;
...
//double ProxOMP::line_search(double *v, double *gw, double *normsq,	double *normsq_search,
							...
							double tau, double *b0,double *b, double *upd_t, double fv,double lambda,double &f_w_)
{
	...
	for(i=0; i<50; i++)
	{
		...
		for(j=0; j+4<m; j+=4)
		{
			...
			alpha_sum[j+1] = max(0.0,1-double(y[j+1]*pred_temp1)); 
			alpha_sum[j+2] = max(0.0,1-double(y[j+2]*pred_temp2)); 
			alpha_sum[j+3] = max(0.0,1-double(y[j+3]*pred_temp3)); 
     ...
     /*  pred_temp1 = 0;
		...
		for(k=0; k<p; k++)
		{
			for(j=0; j< B; j++)
			{
				dw_proj += gw[k*B+j]*(upd_t[k]*SG[k*B+j]-v[k*B+j]);
				dw_diff += (upd_t[k]*SG[k*B+j]-v[k*B+j])*(upd_t[k]*SG[k*B+j]-v[k*B+j]);
				//G_SG    += 0.5*tau*(G[k*B+j]-SG[k*B+j])*(G[k*B+j]-SG[k*B+j]);
				//normSY  += 0.5/tau*gw[k*B+j]*gw[k*B+j];
			}
		}
...
int ProxOMP::mkl_prox_linear_l2svm()
{
	...
	for(t = 0; t < numEpochs; t++)
	{
	   ...
	   //printf("line_search elaps time is %f\n",run_time);
		...
		if (abs(f_w_temp-f_w)/abs(f_w_temp)<eps_inner)
		{
			break;	
		}else
		{
