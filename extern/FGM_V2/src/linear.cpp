...
int save_model_poly(const char *model_file_name, const struct model *model_)
{
	...
	{
		fprintf(fp, "%ld ", model_->solution_->w_FGM[j].index1);
		fprintf(fp, "%ld:%.16g\n", model_->solution_->w_FGM[j].index2,model_->solution_->w_FGM[j].value);
		j++;
	}
	if (param.t==1)
	{
		...
		{
			fprintf(fp, "%ld ", model_->solution_->w_FGM_retrain[j].index1);
			fprintf(fp, "%ld:%.16g\n", model_->solution_->w_FGM_retrain[j].index2,model_->solution_->w_FGM_retrain[j].value);
			j++;
		}
		...
		{
			fprintf(fp, "%ld ", model_->solution_->w_FGM_B[j].index1);
			fprintf(fp, "%ld:%.16g\n", model_->solution_->w_FGM_B[j].index2,model_->solution_->w_FGM_B[j].value);
			j++;
		}
	}
							  ...
							  double Cp, double Cn)
{
	...
	while (iter_idx<4)
	{
		...
		if (abs(obj-obj_temp)/abs(obj)<1e-4)
		{
			printf("success!\n");
			break;
		}
