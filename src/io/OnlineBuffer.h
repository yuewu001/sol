...
namespace BOC {
	...
	template <typename PointType> class OnlineBuffer {
	protected:
	...
	protected:
		...
		ChunkType *head;
		ChunkType *wt_ptr; //pointer to the write location
		ChunkType *rd_ptr; //pointer to the read location
	...
	protected:
		...
		bool CreateBuffer(int buf_size, int chunk_size) {
			...
			this->head = new ChunkType(chunk_size);
			ChunkType *p = this->head;
			for (int i = 1; i < this->buf_size; i++) {
				p->next = new ChunkType(chunk_size);
				p = p->next;
			}
			p->next = this->head;
