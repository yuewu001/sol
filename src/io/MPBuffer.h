...
namespace SOL{
	...
	enum MPBufferType{
		...
		MPBufferType_FALSE = 2, //buffer false predictions
	};
    ...
    template <typename FeatType, typename LabelType> 
		...
		template <typename FeatType, typename LabelType>
		struct MPBuffer_FALSE :public MPBuffer<FeatType, LabelType>{
			MPBuffer_FALSE(size_t bufSize = init_mp_buf_size) :MPBuffer(bufSize){
			}
