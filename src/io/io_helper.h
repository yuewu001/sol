...
namespace BOC{
    template <typename FeatType, typename LabelType>
        DataReader<FeatType, LabelType>* getReader(const string &filename, string dt_type){
            ToLowerCase(dt_type);
            if (dt_type == "libsvm")
                return new BOC::libsvm_io_<FeatType, LabelType>(filename);
            else if (dt_type == "csv")
                return new BOC::csv_io_<FeatType,LabelType>(filename);
            else if (dt_type == "cache")
                return new BOC::libsvm_binary_<FeatType,LabelType>(filename);
            else {
